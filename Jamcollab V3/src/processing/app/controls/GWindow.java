...
/**
 * Objects of this class are separate windows which can be used to hold G4P GUI
 * components or used for drawing or both combined. <br>
 * <br>
 * A number of examples are included in the library and can be found at
 * www.lagers.org.uk
 * 
 * 
 * @author Peter Lager
...
public class GWindow extends Frame implements GConstants, GConstantsInternal {
	...
	/**
	 * Create a window that can be used to hold G4P components or used for
	 * drawing or both together.
	 * 
	 * @param theApplet
	 * @param name
	 * @param x
	 ...
	 *            initial position on the screen
	 * @param w
	 *            width of the drawing area (the frame will be bigger to
	 *            accommodate border)
	 * @param h
	 *            height of the drawing area (the frame will be bigger to
	 *            accommodate border and title bar)
	 * @param noFrame
	 *            if true then the frame has no border
	 * @param mode
	 *            JAVA2D / P2D / P3D / OPENGL
	 */
	public GWindow(PApplet theApplet, String name, int x, int y, int w, int h,
			boolean noFrame, String mode) {
		super(name);
		winName = name;
	 ...
	 *            initial position on the screen
	 * @param image
	 *            background image (used to size window)
	 * @param noFrame
	 *            if true then the frame has no border
	 * @param mode
	 *            JAVA2D / OPENGL
	 */
	public GWindow(PApplet theApplet, String name, int x, int y, PImage image,
			boolean noFrame, String mode) {
		super(name);
		windowCtorCore(theApplet, x, y, image.width, image.height, image,
				noFrame, mode, name);
	}
	 ...
	 * @param mode
	 */
	private void windowCtorCore(PApplet theApplet, int x, int y, int w, int h,
			PImage image, boolean noFrame, String mode, String name) {
		...
		papplet.setPreferredSize(new Dimension(papplet.appWidth,
				papplet.appHeight));
		papplet.setMinimumSize(new Dimension(papplet.appWidth,
				papplet.appHeight));
	...
	public void setOnTop(boolean onTop) {
		try {
			...
			if (G4P.showMessages)
				System.out
						.println("Warning: setOnTop() method will not work when the sketch is run from a remote location.");
		}
	}

	/**
	 * Sets the location of the window. <br>
	 * (Already available from the Frame class - helps visibility of method in
	 * G4P reference)
	 */
	...
	/**
	 * By default the background() method is called to set the background
	 * image/colour every frame. You can switch this off by calling this method
	 * with a parameter value = false.
	 * 
	 * @param auto_clear
	 *            whether to call the background() method or not
	 */
	 ...
	 * GWindow.CLOSE_WINDOW - close this window, if it is the main window it
	 * causes the app to exit <br>
	 * GWindow.EXIT_APP - exit the app, this will cause all windows to close. <br>
	 * 
	 * @param action
	 *            the required close action
	 */
	...
	/**
	 * Get the action to be performed when the user attempts to close the
	 * window.
	 * 
	...
	/**
	 * This method guarantees that the window is closed by overriding the
	 * KEEP_OPEN action-on-close and will fire a WindowClosing event to be
	 * captured by the GWindow$GWindowAdapter object. <br>
	 * There are 2 possible actions depending on the currently specified
	 * action-on-close. <br>
	 ...
	 * method that returns void and has three parameters GWinApplet, GWinData
	 * and a MouseEvent
	 * 
	 * @param obj
	 *            the object to handle the event
	...
	public void addMouseHandler(Object obj, String methodName) {
		try {
			mouseHandlerMethod = obj.getClass().getMethod(
					methodName,
					new Class<?>[] { GWinApplet.class, GWinData.class,
							MouseEvent.class });
			...
			GMessenger.message(NONEXISTANT, new Object[] {
					this,
					methodName,
					new Class<?>[] { GWinApplet.class, GWinData.class,
							MouseEvent.class } });
	...
	public void addKeyHandler(Object obj, String methodName) {
		try {
			keyHandlerMethod = obj.getClass().getMethod(
					methodName,
					new Class<?>[] { GWinApplet.class, GWinData.class,
							KeyEvent.class });
	...
	/**
	 * Window adapter class that remembers the window it belongs to so it can be
	 * used to mark it for closure if required.
	 * 
	 * @author Peter Lager
	 */
