...
public class GTextField extends GEditableTextControl {
	 ...
	 * @param sbPolicy
	 */
	public GTextField(PApplet theApplet, float p0, float p1, float p2,
			float p3, int sbPolicy) {
	...
	/**
	 * Set the styled text for this textfield after ensuring that all EOL
			...
			if (endTLHI != null) {
				if (ptx > caretX) { // Scroll to the left (text moves right)
					...
					if (ptx < 0)
						ptx = 0;
					horzScroll = true;
				} else if (ptx < max_ptx) { // Scroll to the right (text moves
											// left)?
					ptx += HORZ_SCROLL_RATE;
					if (ptx > max_ptx)
						ptx = max_ptx;
				...
				// Ensure that we show as much text as possible keeping the
				// caret in view
				// This is particularly important when deleting from the end of
				// the text
