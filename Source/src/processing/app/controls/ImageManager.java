...
public class ImageManager {
	...
	/**
	 * Make multiple images from a given image. This method creates a 2D array
	 * (size [nCols, nRows] ) of PImage objects.
	 * 
	 * @param app
	 * @param img
	 *            the tiled image
	 ...
	 */
	public static PImage[][] makeTiles2D(PApplet app, PImage img, int nCols,
			int nRows) {
		...
		for (int y = 0; y < nRows; y++) {
			for (int x = 0; x < nCols; x++) {
				...
				imageTiles[x][y].copy(img, x * tileW, y * tileH, tileW, tileH,
						0, 0, tileW, tileH);
			}
		}
		return imageTiles;
	...
	/**
	 * Make multiple images from a given image. This method creates a 1D array
	 * of PImage objects. The order is left-right and top-down.
	 * 
	 * @param app
	 * @param img
	 *            the tiled image
	 ...
	 */
	public static PImage[] makeTiles1D(PApplet app, PImage img, int nCols,
			int nRows) {
		PImage[] imageTiles = new PImage[nCols * nRows];
		int tileW = img.width / nCols;
		int tileH = img.height / nRows;
		...
		for (int y = 0; y < nRows; y++) {
			for (int x = 0; x < nCols; x++) {
				imageTiles[tileNo] = app
						.createImage(tileW, tileH, PApplet.ARGB);
				imageTiles[tileNo].copy(img, x * tileW, y * tileH, tileW,
						tileH, 0, 0, tileW, tileH);
				tileNo++;
			}
		}
		return imageTiles;
