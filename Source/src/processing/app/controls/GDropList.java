...
public class GDropList extends GTextBase {
	 ...
	 * 
	 * @param theApplet
	 *            the applet that will display this component.
	 * @param p0
	 * @param p1
	 * @param p2
	 * @param p3
	 ...
	 * @param p2
	 * @param p3
	 * @param dropListMaxSize
	 *            the maximum number of element to appear in the drop down list
	 */
	public GDropList(PApplet theApplet, float p0, float p1, float p2, float p3,
			int dropListMaxSize) {
		...
		itemHeight = height / (dropListMaxSize + 1); // make allowance for
														// selected text at top
		...
		hotspots = new HotSpot[] {
				new HSrect(LIST_SURFACE, 0, itemHeight + 1, width - 11, height
						- itemHeight - 1), // text list area
				new HSrect(CLOSED_SURFACE, 0, 0, width - buttonWidth,
						itemHeight) // selected text display area
		};

		createEventHandler(G4P.sketchApplet, "handleDropListEvents",
				new Class<?>[] { GDropList.class, GEvent.class }, new String[] {
						"list", "event" });
	...
	/**
	 * Use this to set or change the list of items to appear in the list. If you
	 * enter an invalid selection index then it is forced into the valid range. <br>
	...
	public void setItems(String[] list, int selected) {
		...
		startItem = (selItem >= dropListMaxSize) ? selItem - dropListMaxSize
				+ 1 : 0;
	...
	public void setSelected(int selected) {
		if (selected >= 0 && selected < sitems.length) {
			selItem = selected;
			startItem = (selItem >= dropListMaxSize) ? selItem
					- dropListMaxSize + 1 : 0;
	...
	public boolean isOver(float x, float y) {
		...
		return (!expanded) ? currSpot == CLOSED_SURFACE
				: currSpot == CLOSED_SURFACE | currSpot == LIST_SURFACE;
	}
	...
	protected void updateBuffer() {
		if (bufferInvalid) {
			...
			if (expanded) {
				...
				buffer.rect(0, itemHeight, width, itemHeight
						* dropListActualSize);
			}
