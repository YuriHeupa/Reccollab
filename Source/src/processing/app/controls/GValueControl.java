...
public abstract class GValueControl extends GAbstractControl {
	...
	public GValueControl(PApplet theApplet, float p0, float p1, float p2,
			float p3) {
	 ...
	 * specific actions when the value changes. Override this in GSlider to
	 * change the hotshot poaition.
	 */
	...
	/**
	 * For DECIMAL values this sets the number of decimal places to be
	 * displayed.
	 * 
	 * @param nd
	 *            must be >= 1 otherwise will use 1
	 */
	...
	/**
	 * Make epsilon to match the value of 1 pixel or the precision which ever is
	 * the smaller
	 */
	 ...
	 * ($), fps etc. <br>
	 * Do not use long labels such as 'miles per hour' as these take a lot of
	 * space and can look messy.
	 * 
	 * @param units
	 *            for example kg, m, ($), fps
	 */
	 ...
	 * 
	 * @param numberFormat
	 *            G4P.INTEGER, G4P.DECIMAL orG4P. EXPONENT
	 * @param precision
	 *            must be >= 1
	 */
	 ...
	 * 
	 * @param numberFormat
	 *            G4P.INTEGER, G4P.DECIMAL or G4P.EXPONENT
	 */
	...
	/**
	 * The number of ticks must be >= 2 since 2 are required for the slider
	 * limits.
	 * 
	 * @param noOfTicks
	 *            the nbrTicks to set
	...
	/**
	 * Specify whether the values are to be constrained to the tick marks or
	 * not. It will automatically display tick marks if set true.
	 * 
	 * @param stickToTicks
	 *            true if you want to constrain the values else false
	 */
	 ...
	 *            show current value
	 * @param limits
	 *            show min and max values (limits)
	 */
	public void setShowDecor(boolean opaque, boolean ticks, boolean value,
			boolean limits) {
		setShowValue(value);
		bufferInvalid = true;
		setOpaque(opaque);
