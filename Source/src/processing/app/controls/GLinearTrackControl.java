...
public abstract class GLinearTrackControl extends GValueControl {
	...
	public GLinearTrackControl(PApplet theApplet, float p0, float p1, float p2,
			float p3) {
	 ...
	 * appropriate. <br>
	 * Acceptable values are G4P.ORIENT_LEFT, G4P.ORIENT_RIGHT or
	 * G4P.ORIENT_TRACK <br>
	 * If an invalid value is passed the ORIENT_TRACK is used.
	 * 
	 * @param orient
	 *            the orientation of the number labels
	 */
	 ...
	 * The array passed must have a minimum of 2 elements and each label
	 * (element) must have at least 1 character. If these two conditions are not
	 * met then the call to this method will be ignored and no changes are made.
	 * 
	 * @param tickLabels
	 *            an array of strings for the labels
	 */
	...
	/**
	 * Set whether the tick marks are to be displayed or not. Then recalculate
	 * the track offset for the value and limit text.
	 * 
	 * @param showTicks
	...
	/**
	 * The offset is the distance the value/labels are drawn from the centre of
	 * the track. <br>
	...
	/**
	 * If we are using labels then this will get the label text associated with
	 * the current value. <br>
	 * If labels have not been set then return null
	 */
	public String getValueS() {
		// Use the valueTarget rather than the valuePos since intermediate
		// values
		// have no meaning in this case.
		int idx = Math.round(startLimit + (endLimit - startLimit)
				* parametricTarget);
		return (labels == null) ? getNumericDisplayString(getValueF())
				: labels[idx].getPlainText();
	}
	...
	public void mouseEvent(MouseEvent event) {
		...
		case MouseEvent.PRESS:
			if (focusIsWith != this && currSpot > -1 && z >= focusObjectZ()) {
				...
				status = (downHotSpot == THUMB_SPOT) ? PRESS_CONTROL
						: OFF_CONTROL;
	...
	protected void drawLabels() {
		...
		case ORIENT_TRACK:
			for (int i = 0; i < labels.length; i++) {
				line = labels[i].getLines(g2d).getFirst().layout;
				px = (i * deltaX - 0.5f) * trackLength - 0.5f
						* line.getVisibleAdvance();
				py = trackOffset + line.getAscent();
				line.draw(g2d, px, py);
			}
			break;
