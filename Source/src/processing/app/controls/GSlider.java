...
public class GSlider extends GLinearTrackControl {
	...
	public GSlider(PApplet theApplet, float p0, float p1, float p2, float p3,
			float tr_width) {
		super(theApplet, p0, p1, p2, p3);
		trackWidth = tr_width;
		trackDisplayLength = width - 2 * TINSET;
		trackLength = trackDisplayLength - trackWidth;
		track = new RoundRectangle2D.Float(-trackDisplayLength / 2,
				-trackWidth / 2, trackDisplayLength, trackWidth, trackWidth,
				trackWidth);
		...
		hotspots = new HotSpot[] {
				new HScircle(THUMB_SPOT, width / 2 + (parametricPos - 0.5f)
						* trackLength, height / 2, trackWidth / 2), // thumb
	...
	protected void updateBuffer() {
		if (bufferInvalid) {
			...
			buffer.noStroke();
			buffer.ellipse((parametricPos - 0.5f) * trackLength, 0, trackWidth,
					trackWidth);
			// Draw track border
