...
public class GWindowInfo implements PConstants, GConstants, GConstantsInternal {
    ...
    // These next two lists are for controls that are to be added or remove
    // since these
    ...
    public GWindowInfo(PApplet papplet) {
		...
		/*
         * The WinInfo object is responsible for capturing events from
		 * Processing and passing them onto the GWindow objects and the
		 * controls.
		 */
     ...
     * <p>
     * If this is a secondary window then it should call the draw method for the
     * window.
     * <p>
     * Should call the user defined method for the the draw method for the
     * actual GWindow object.
     *
     * @param event
     */
     ...
     */
    public void pre() {
        if (GAbstractControl.controlToTakeFocus != null
                && GAbstractControl.controlToTakeFocus.getPApplet() == app) {
    ...
    public void post() {
        if (G4P.cursorChangeEnabled) {
            if (GAbstractControl.cursorIsOver != null) // &&
                // GAbstractControl.cursorIsOver.getPApplet()
                // == app)
