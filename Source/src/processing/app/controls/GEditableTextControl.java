...
public abstract class GEditableTextControl extends GTextBase {
	...
	protected int endChar = -1, startChar = -1, pos = endChar, nbr = 0,
			adjust = 0;
	...
	public GEditableTextControl(PApplet theApplet, float p0, float p1,
			float p2, float p3, int scrollbars) {
	...
	/**
	 * Give up focus but if the text is only made from spaces then set it to
	 * null text. <br>
	 ...
	 */
	public void addStyle(TextAttribute attr, Object value, int charStart,
			int charEnd) {
		if (stext != null) {
	...
	public String getSelectedText() {
		...
		int ss = startSelTLHI.tli.startCharIndex
				+ startSelTLHI.thi.getInsertionIndex();
		int ee = endSelTLHI.tli.startCharIndex
				+ endSelTLHI.thi.getInsertionIndex();
		String s = stext.getPlainText().substring(ss, ee);
		return s;
	}

	/**
	 * If some text has been selected then set the style. If there is no
	 * selection then the text is unchanged.
	 * 
	 * 
	 * @param style
	 */
	...
	public boolean hasSelection() {
		return (startTLHI.tli != null && endTLHI.tli != null && startTLHI
				.compareTo(endTLHI) != 0);
	}
	 ...
	 * If text editing is being disabled and the control has focus then it is
	 * forced to give up that focus. <br>
	 * This might be useful if you want to use a GTextArea control to display
	 * large amounts of text that needs scrolling (so cannot use a GLabel) but
	 * must not change e.g. a user instruction guide.
	 * 
	 * @param enableTextEdit
	 *            false to disable keyboard input
	 */
	...
	public void keyEvent(KeyEvent e) {
		...
		if (focusIsWith == this && endTLHI != null) {
			...
			endChar = endTLHI.tli.startCharIndex
					+ endTLHI.thi.getInsertionIndex();
			startChar = (startTLHI != null) ? startTLHI.tli.startCharIndex
					+ startTLHI.thi.getInsertionIndex() : endChar;
			pos = endChar;
			nbr = 0;
			adjust = 0;
			...
			// Select either keyPressedProcess or keyTypeProcess. These two
			// methods are overridden in child classes
			if (keyID == KeyEvent.PRESS) {
				keyPressedProcess(keyCode, keyChar, shiftDown, ctrlDown);
				setScrollbarValues(ptx, pty);
			} else if (keyID == KeyEvent.TYPE) { // && e.getKey() !=
	...
	/**
	 * Do not call this directly. A timer calls this method as and when
	 * required.
	 */
	...
	public void vsbEventHandler(GScrollbar scrollbar, GEvent event) {
		keepCursorInView = false;
		pty = vsb.getValue()
				* (stext.getTextAreaHeight() + 1.5f * stext.getMaxLineHeight());
		bufferInvalid = true;
	}
	...
	public boolean saveText(String fname) {
		if (stext == null)
			return false;
		if (hasSelection()) {
			stext.startIdx = startTLHI.tli.startCharIndex
					+ startTLHI.thi.getInsertionIndex();
			stext.endIdx = endTLHI.tli.startCharIndex
					+ endTLHI.thi.getInsertionIndex();
		} else {
	...
	public boolean loadText(String fname) {
		...
		if (stext.startIdx >= 0) { // we have a selection
			...
			if (pInLayout == 0)
				...
				startTLHI.thi = startTLHI.tli.layout
						.getNextRightHit(pInLayout - 1);
			// Selection ends at ...
