...
import processing.app.controls.HotSpot.HSarc;
import processing.app.controls.HotSpot.HScircle;
...
public class GKnob extends GValueControl {
	 ...
	 * The knob has two zones the outer bezel and the inner gripper. The radius
	 * of the outer bezel is calculated from <br>
	 * 
	 * <pre>bezel radius = min(width, height)/2 - 2
	 * 
	 * <pre>
	 * <br>
	 ...
	 * 
	 * <br>
	 * The gripAmount should be in te range 0.0 to 1.0 inclusive. The actual
	 * value will be constrained to that range. <br>
	 * 
	 * @param theApplet
	 * @param p0
	 * @param p1
	 ...
	 *            must be >=0.0 and <=1.0
	 */
	public GKnob(PApplet theApplet, float p0, float p1, float p2, float p3,
			float gripAmount) {
		...
		hotspots = new HotSpot[] { new HScircle(1, width / 2, height / 2,
				gripRadius) };
		z = Z_SLIPPY;
	...
	protected void calculateHotSpot() {
		...
		if (mouseOverArcOnly)
			hotspots[0] = new HSarc(1, width / 2, height / 2, overRad,
					startAng, endAng); // over grip
		else
	 ...
	 * it will only respond when ver the arc made by the start and end angles.
	 * If false it will be the full circle.
	 * 
	 * @param arcOnly
	 */
	 ...
	 *            draw arc only?
	 * @param overfullsize
	 *            include bezel in mouse over calculations?
	 */
	public void setArcPolicy(boolean over_arc_only, boolean draw_arc_only,
			...
			if (focusIsWith == this) {
				...
				if (mouseAngle != lastMouseAngle) {
					...
					if (deltaMangle < -180)
						...
						deltaMangle -= 360;
					// Calculate and adjust new needle angle so it is in the
					// range aLow >>> aHigh
					angleTarget = constrainToTurnRange(angleTarget
							+ deltaMangle);
	...
	protected void updateBuffer() {
		...
		if (bufferInvalid) {
			...
			if (bezelWidth > 0) {
				...
				if (drawArcOnly)
					buffer.arc(0, 0, 2 * bezelRadius, 2 * bezelRadius,
							PApplet.radians(startAng), PApplet.radians(endAng));
				else
				...
				if (showTicks) {
					...
					for (int t = 0; t < nbrTicks; t++) {
						tickLength = gripRadius
								+ ((t == 0 || t == nbrTicks - 1) ? bezelWidth
										: bezelWidth * 0.8f);
						a = Math.toRadians(startAng + t * deltaA);
						sina = Math.sin(a);
						cosa = Math.cos(a);
						buffer.line((float) (gripRadius * cosa),
								(float) (gripRadius * sina),
								(float) (tickLength * cosa),
								(float) (tickLength * sina));
					}
				}
				// draw track?
				if (showTrack) {
					...
					buffer.arc(0, 0, 2 * (gripRadius + bezelWidth * 0.5f),
							2 * (gripRadius + bezelWidth * 0.5f),
							PApplet.radians(startAng),
							PApplet.radians(anglePos));
				}
			}
			...
			if (drawArcOnly)
				buffer.arc(0, 0, 2 * gripRadius, 2 * gripRadius,
						PApplet.radians(startAng), PApplet.radians(endAng));
			else
			...
			sina = Math.sin(a);
			cosa = Math.cos(a);
			buffer.line(0, 0, (float) (gripRadius * cosa),
					(float) (gripRadius * sina));
			buffer.endDraw();
		}
	}
	 ...
	 * This gets the sensitivity to be used in modes CTRL_HORIZONTAL and
	 * CTRL_VERTICAL <br>
	 * A value of 1 is 1 degree per pixel and a value of 2 is 2 degrees per
	 * pixel. <br>
	 * 
	 * @param sensitivity
	 ...
	 * 
	 * @param px
	 *            the distance from the knob centre in the x direction
	 * @param py
	 *            the distance from the knob centre in the y direction
	 ...
	 * 
	 * @param start
	 *            the range start angle in degrees
	 * @param end
	 *            the range end angle in degrees
	 */
	...
	/**
	 * Calculate the angle to the knob centre making sure it is in the range
	 * 0-360
	 * 
	 * @param px
	 * @param py
