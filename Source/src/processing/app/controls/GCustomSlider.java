...
public class GCustomSlider extends GLinearTrackControl {
	 ...
	 * @param p3
	 */
	public GCustomSlider(PApplet theApplet, float p0, float p1, float p2,
			float p3) {
	 ...
	 * @param p3
	 * @param skin
	 *            the name of the skin (this is also the name of the folder
	 *            holding the images)
	 */
	public GCustomSlider(PApplet theApplet, float p0, float p1, float p2,
			float p3, String skin) {
		...
		hotspots = new HotSpot[] {
				new HSalpha(THUMB_SPOT, width / 2 + (parametricPos - 0.5f)
						* trackLength, height / 2, thumb, PApplet.CENTER), // thumb
				new HSrect(TRACK_SPOT, (width - trackLength) / 2,
						(height - trackWidth) / 2, trackLength, trackWidth), // track
		};
		opaque = false;
		z = Z_SLIPPY;
	 ...
	 * 
	 * @param skin
	 *            the name of the folder holding the graphics for this slider
	 */
	public void setStyle(String skin) {
		loadSkin(skin);
		float maxEndLength = Math.max(leftEnd.width, rightEnd.width);
		maxEndLength = Math.max(maxEndLength, 10); // make sure we have enough
													// to show limits value
		trackLength = Math.round(width - 2 * maxEndLength - TINSET);
		trackDisplayLength = trackLength + 2
				* Math.min(leftEnd.width, rightEnd.width);
	...
	protected void updateBuffer() {
		if (bufferInvalid) {
			...
			case PRESS_CONTROL:
			case DRAG_CONTROL:
				buffer.image(thumb_mouseover, (parametricPos - 0.5f)
						* trackLength, 0);
				break;
			}
	...
	private void loadSkin(String style) {
		...
		if (!style.equalsIgnoreCase(style_used))
			System.out.println("Unable to load the skin " + style
					+ " using default '" + DEFAULT_SLIDER_STYLE
					+ "' style instead");
	}
	...
	private boolean loadStyle_FromSketch(String style) {
		...
		int fcount = 0;
		String[] names = new String[] { "centre.", "end_left.", "end_right.",
				"handle.", "handle_mouseover." };
	...
	private boolean loadSkin_AppletInBrowser(String style) {
		...
		boolean found = !(leftEnd == null || rightEnd == null || thumb == null
				|| thumb_mouseover == null || centre == null);
		...
		if (!found) {
			System.out.println("Unable to load the skin " + style
					+ " check the ");
			System.out
					.println("skin name used and ensure all the image files are present.");
