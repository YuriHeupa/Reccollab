...
public class Login extends BaseObject {
    ...
    public void Awake() {
        needPass = false;
        StatusMessage = new GLabel(Reccollab.app, 180, 234, 220, 40);
        StatusMessage.setText("");
        StatusMessage.setLocalColorScheme(GCScheme.RED_SCHEME);
        StatusMessage.setOpaque(false);
        StatusMessage.setVisible(false);
        LoginLabel = new GLabel(Reccollab.app, 140, (needPass ? 160 : 200), 60, 20);
        LoginLabel.setText("Login:");
        LoginLabel.setTextBold();
        LoginLabel.setOpaque(false);
        LoginLabel.setVisible(false);
        LoginLabel.setTextAlign(GAlign.LEFT, GAlign.CENTER);
        PasswordLabel = new GLabel(Reccollab.app, 136, 200, 45, 20);
        PasswordLabel.setText("Senha:");
        PasswordLabel.setTextBold();
        PasswordLabel.setOpaque(false);
        PasswordLabel.setVisible(false);
        LoginInput = new GTextField(Reccollab.app, 180, (needPass ? 160 : 200), 260, 20, G4P.SCROLLBARS_NONE);
        LoginInput.setDefaultText("Login...");
        LoginInput.setOpaque(true);
        LoginInput.setVisible(false);
        PasswordInput = new GTextField(Reccollab.app, 180, 200, 260, 20, G4P.SCROLLBARS_NONE);
        ...
        LoginButton = new GButton(Reccollab.app, 250, 280, 80, 30);
            ...
            if (!Utils.isAlphanumeric(user)) {
                ...
                if (needPass) {
                    if (CurrentPassword.isEmpty()) {
                        SM("Digite sua senha de usu�rio");
                        return;
                    } else {
                        if (!Utils.DoLogin(user, CurrentPassword)) {
                            SM("Nome de usu�rio e/ou senha inv�lidos");
                            return;
                        }
                    }
                }
