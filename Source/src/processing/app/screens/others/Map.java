...
public class Map extends BaseObject {
    ...
    public final static Vector2D screenOffsetSize = new Vector2D(Reccollab.app.width - 5, 294);
    ...
    @Override
    public void Awake() {
        Level1Button = new GButton(Reccollab.app, 24, 312, 160, 16);
        ...
        Level3Button = new GButton(Reccollab.app, 408, 312, 160, 16);
        ...
        Level2Button = new GButton(Reccollab.app, 216, 312, 160, 16);
        ...
        HintLabel = new GLabel(Reccollab.app, 8, 336, 584, 16);
        ...
        Reccollab.app.addMouseWheelListener(new java.awt.event.MouseWheelListener() {
            public void mouseWheelMoved(java.awt.event.MouseWheelEvent evt) {
                mouseWheel(evt.getWheelRotation());
            }
        });
        Reccollab.app.registerMethod("mouseEvent", this);
    }
    ...
    public Vector2D mousePosition() {
        return new Vector2D(Reccollab.app.mouseX, Reccollab.app.mouseY);
    }
        ...
        if (show) {
            if (view.isVisible()) {
                ...
                if (PIN.getPos().x != -1 && PIN.getPos().y != -1
                    ...
                    Reccollab.app.image(Assets.mapPIN, getPinPos().x - (Assets.mapPIN.width / 2),
                            getPinPos().y - Assets.mapPIN.height);
                }

                if (HoverZoomIn())
                    Reccollab.app.tint(255, 50, 50);
                if (getCurrentMap().getLevel() < getCurrentMap().getLevelsCount() - 1)
                    Reccollab.app.image(Assets.zoomIn, screenOffsetPos.x + 10, screenOffsetPos.y + 10);
                Reccollab.app.tint(255);
                if (HoverZoomOut())
                    Reccollab.app.tint(255, 50, 50);
                if (getCurrentMap().getLevel() > 0)
                    Reccollab.app.image(Assets.zoomOut, screenOffsetPos.x + 10, screenOffsetPos.y + 35);
                Reccollab.app.tint(255);
            }
            updateCurrentMap();
        }
    ...
    private void drawZoomRect(float size) {
        Reccollab.app.rectMode(PConstants.CENTER);
        Reccollab.app.stroke(255, 0, 0);
        Reccollab.app.noFill();
        ...
        Reccollab.app.rect(mouseZoom.x, mouseZoom.y, newWidth * size, newHeight * size);
    }
