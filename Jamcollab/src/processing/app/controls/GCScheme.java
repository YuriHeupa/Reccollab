 ...
 * 
 * It loads an image file with all the colors used by the various colour
 * schemes. <br>
 * It will search for a file for a user defined scheme (user_gui_palette.png)
 * and if it can't find it it will use the library default scheme
...
public class GCScheme implements GConstants {
	...
	/**
	 * Set the color scheme to one of the preset schemes BLUE / GREEN / RED /
	 * PURPLE / YELLOW / CYAN / BROWN or if you have created your own schemes
	 * following the instructions at
	 * gui4processing.lagers.org.uk/colorscheme.html then you can enter the
	 * appropriate numeric value of the scheme.
	 * 
	 * @param schemeNo
	...
	public static void makeColorSchemes(PApplet app) {
		...
		for (int p = 0; p < 16; p++)
			for (int c = 0; c < 16; c++) {
				...
				palettes[p][c] = col;
				jpalettes[p][c] = new Color((col >> 16) & 0xff,
						(col >> 8) & 0xff, col & 0xff);
			}
	}

	/**
	 * This method is called by the G4P GUI Builder tool when there is no sketch
	 * = no PApplet object to use
	 */
	public static void makeColorSchemes() {
		...
		PImage image = new PImage(
				(new javax.swing.ImageIcon(new GCScheme().getClass()
						.getResource("/data/default_gui_palette.png")))
						.getImage());
		// Now make the palletes
		palettes = new int[16][16];
		jpalettes = new Color[16][16];
