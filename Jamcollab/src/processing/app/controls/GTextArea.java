...
import processing.app.controls.StyledString.TextLayoutHitInfo;
 ...
 * control also allows default text, horizontal and vertical scrollbars.
 * 
...
public class GTextArea extends GEditableTextControl {
	...
	/**
	 * Create a text field with the given scrollbar policy and a text wrap width
	 * to fit the control. <br>
	 ...
	 */
	public GTextArea(PApplet theApplet, float p0, float p1, float p2, float p3,
			int sbPolicy, int wrapWidth) {
		super(theApplet, p0, p1, p2, p3, sbPolicy);
		children = new LinkedList<GAbstractControl>();
		tx = ty = pad;
		tw = width - 2 * pad
				- ((scrollbarPolicy & SCROLLBAR_VERTICAL) != 0 ? 18 : 0);
		th = height - 2 * pad
				- ((scrollbarPolicy & SCROLLBAR_HORIZONTAL) != 0 ? 18 : 0);
		// The text wrap width is based on the width of the text display area
		// unless
		// some other value is specified.
		this.wrapWidth = (wrapWidth == Integer.MAX_VALUE) ? (int) tw
				: wrapWidth;
		// Clip zone
	 ...
	 * displayed in italic whenever it is empty. It will also be wrapped to the
	 * control size.
	 * 
	 * @param dtext
	 */
	 ...
	 * 
	 * @param attr
	 *            the text attribute to add
	 * @param value
	 *            value of the text attribute
	 * @param lineNo
	 *            the line number (starts at 0)
	 * @param charStart
	 *            the position of the first character to apply the attribute
	 * @param charEnd
	 *            the position after the last character to apply the attribute
	 */
	...
	/**
	 * Clears all text attribute from a range of characters on a particular
	 * line. If charEnd is past the EOL then the attributes will be cleared to
	 * the end-of-line.
	 * 
	 * @param lineNo
	 *            the line number (starts at 0)
	...
	/**
	 * Add text to the end of the current text. This is useful for a logging'
	 * type activity.
	 * 
	 * @param extraText
	 *            the text to append
	...
	public void appendText(String extraText) {
		...
		endTLHI.thi = endTLHI.tli.layout
				.getNextRightHit(endTLHI.tli.nbrChars - 1);
		startTLHI.copyFrom(endTLHI);
		calculateCaretPos(endTLHI);
		if (vsb != null) {
	...
	protected void updateBuffer() {
		if (bufferInvalid) {
			...
			for (TextLayoutInfo lineInfo : lines) {
				...
				if (hasSelection() && lineInfo.compareTo(startSelTLHI.tli) >= 0
						&& lineInfo.compareTo(endSelTLHI.tli) <= 0) {
					int ss = 0;
					ss = (lineInfo.compareTo(startSelTLHI.tli) == 0) ? startSelTLHI.thi
							.getInsertionIndex() : 0;
					int ee = endSelTLHI.thi.getInsertionIndex();
					ee = (lineInfo.compareTo(endSelTLHI.tli) == 0) ? endSelTLHI.thi
							.getInsertionIndex() : lineInfo.nbrChars - 1;
		...
		if (keepCursorInView) {
			...
			if (endTLHI != null) {
				...
				if (vertScroll && vsb != null)
					vsb.setValue(pty
							/ (stext.getTextAreaHeight() + 1.5f * stext
									.getMaxLineHeight()));
			}
	...
	public void draw() {
		...
		if (focusIsWith == this && showCaret && endTLHI != null) {
			...
			if (x_left >= 0 && x_left <= tw && y_top >= 0 && y_bot <= th) {
				...
				winApp.line(tx + x_left, ty + Math.max(0, y_top), tx + x_left,
						ty + Math.min(th, y_bot));
			}
		}
	...
	protected void keyTypedProcess(int keyCode, char keyChar,
			boolean shiftDown, boolean ctrlDown) {
		...
		if (ascii >= 32 && ascii < 127) {
			...
			if (tabManager != null) {
				boolean result = (shiftDown) ? tabManager.prevControl(this)
						: tabManager.nextControl(this);
				if (result) {
					startTLHI.copyFrom(endTLHI);
					return;
				}
		...
		// If we have emptied the text then recreate a one character string
		// (space)
		if (stext.length() == 0) {
	...
	protected boolean moveCaretEndOfLine(TextLayoutHitInfo currPos) {
		...
		currPos.thi = currPos.tli.layout
				.getNextRightHit(currPos.tli.nbrChars - 1);
		return true;
	}
	...
	protected boolean moveCaretEndOfText(TextLayoutHitInfo currPos) {
		if (currPos.tli.lineNo == stext.getNbrLines() - 1
				&& currPos.thi.getCharIndex() == currPos.tli.nbrChars - 1)
	...
	/**
	 * Move caret left by one character. If necessary move to the end of the
	 * line above
	 * 
	...
	/**
	 * Move caret right by one character. If necessary move to the start of the
	 * next line
	 * 
	 * @return true if caret was moved else false
	 */
	...
	/**
	 * Move the insertion point (caret) to the specified line and character. If
	 * the position is invalid then the caret is not moved. The text will be
	 * scrolled so that the caret position is visible.
	 * 
	 * @param lineNo
	 *            the line number (starts at 0)
	 * @param charNo
	 *            the character position on the line (starts at 0)
	 */
	public void moveCaretTo(int lineNo, int charNo) {
		try {
			TextLayoutHitInfo thli = stext.getTLHIforCharPosition(lineNo,
					charNo);
			if (thli != null) {
