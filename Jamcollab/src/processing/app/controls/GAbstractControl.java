...
public abstract class GAbstractControl implements PConstants, GConstants,
	...
	/*
	 * INTERNAL USE ONLY This holds a reference to the GComponent that currently
	 * has the focus. A component loses focus when another component takes focus
	 * with the takeFocus() method. The takeFocus method should use
	 * focusIsWith.loseFocus() before setting its value to the new component.
	 */
	...
	/*
	 * INTERNAL USE ONLY Keeps track of the component the mouse is over so the
	 * mouse cursor can be changed if we wish.
	 */
	...
	/*
	 * A list of child GComponents added to this component Created and used by
	 * GPanel and GCombo classes
	 */
	...
	protected int timeHovered = 0;
	/*
	 * Position over control corrected for any transformation. <br> [0,0] is top
	 * left corner of the control. This is used to determine the mouse position
	 * over any particular control or part of a control.
	 */
	...
	/*
	 * Is the component available for mouse and keyboard events. This is only
	 * used internally to prevent user input being processed during animation.
	...
	/*
	 * Specify the PImage that contains the image{s} to be used for the button's
	 * state. <br> This image may be a composite of 1 to 3 images tiled
	 * horizontally.
	 * 
	 * @param img
	 * 
	...
	/*
	 * Base constructor for ALL control actors. It will set the position and size
	 * of the control based on controlMode. <br> Since this is an abstract class
	 * it is not possible to use it directly
	 */
	public GAbstractControl(PApplet theApplet, float p0, float p1, float p2,
			float p3) {
		// If this is the first control to be created then theAapplet must be
		// the sketchApplet
	 ...
	 * This means that single changes need not trigger a full redraw to buffer. <br>
	 ...
	 * method on the affected control, and report it as an issue <a href =
	 * 'http://code.google.com/p/gui4processing/issues/list'> here</a><br>
	 * Thanks
	 */
	...
	/**
	 * Return true if Determines if a particular pixel position is over the panel after 500 ms
	 * 
	...
	public boolean isTooltip() {
		...
		if(currSpot >= 0)
			...
			timeHovered = 0;
		return (timeHovered > Assets.ConfigDAO.getIntData("TOOLTIP_TIME_MS", 0)) && visible;
	}
	 ...
	 *            if do do the same for all descendants
	 */
	 ...
	 * 
	 * @param alpha
	 *            value in the range 0 (transparent) to 255 (opaque)
	 */
	...
	/**
	 * Set the transparency of the component and make it unavailable to mouse
	 * and keyboard events if below the threshold. Child controls are ignored? <br>
	...
	public PGraphics getSnapshot() {
		if (buffer != null) {
			updateBuffer();
			PGraphicsJava2D snap = (PGraphicsJava2D) winApp.createGraphics(
					buffer.width, buffer.height, PApplet.JAVA2D);
			snap.beginDraw();
	 ...
	 * @param param_names
	 *            that names of the parameters (used for error messages only)
	 */
	@SuppressWarnings("rawtypes")
	protected void createEventHandler(Object handlerObj, String methodName,
			Class[] param_classes, String[] param_names) {
		try {
			eventHandlerMethod = handlerObj.getClass().getMethod(methodName,
					param_classes);
			eventHandlerObject = handlerObj;
			eventHandlerMethodName = methodName;
		} catch (Exception e) {
			GMessenger.message(MISSING, new Object[] { this, methodName,
					param_classes, param_names });
	...
	public void addEventHandler(Object obj, String methodName) {
		try {
			...
			eventHandlerMethodName = methodName;
			eventHandlerMethod = obj.getClass().getMethod(methodName,
					new Class<?>[] { this.getClass(), GEvent.class });
		} catch (Exception e) {
	...
	protected void fireEvent(Object... objects) {
		if (eventHandlerMethod != null) {
			try {
				...
				GMessenger.message(EXCP_IN_HANDLER, new Object[] {
						eventHandlerObject, eventHandlerMethodName, e });
			}
		}
	}
	 ...
	 *            the control to add.
	 * @param x
	 *            the leftmost or centre position depending on controlMode
	 * @param y
	 *            the topmost or centre position depending on controlMode
	 * @param angle
	 *            the rotation angle (replaces any the angle specified in
	 *            control)
	 */
	 ...
	 * a combo expands it might cover a button. <br>
	...
	public static class Z_Order implements Comparator<GAbstractControl> {
		...
		public int compare(GAbstractControl c1, GAbstractControl c2) {
			if (c1.z != c2.z)
				...
				return new Integer((int) -c1.y).compareTo(new Integer(
						(int) -c2.y));
		}
