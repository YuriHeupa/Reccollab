 ...
 * 
 * To simplify event handling the button only fires off CLICKED events when the
 ...
 * </pre>
 * 
 * enables the other 2 events for button <b>button1</b>. A PRESSED event is
 * created if the mouse button is pressed down over the button face, the CLICKED
 * event is then generated if the mouse button is released over the button face.
 * Releasing the button off the button face creates a RELEASED event. This is
 * included for completeness since it is unlikely you will need to detect these
 * events for this type of control. <br>
 * 
 * 
 * @author Peter Lager
 * 
...
public class GImageToggleButton extends GAbstractControl {
	 ...
	 *            vertical position of the control
	 * @param offPicture
	 *            the filename of bitmap containing toggle state pictures
	 * @param nbrCols
	 ...
	 */
	public GImageToggleButton(PApplet theApplet, float p0, float p1,
			String offPicture, int nbrCols, int nbrRows) {
	 ...
	 *            the filename of bitmap containing toggle state pictures
	 * @param overPicture
	 *            the filename of bitmap containing mouse-over button toggle
	 *            state pictures
	 * @param nbrCols
	 *            number of tiles horizontally
	 */
	public GImageToggleButton(PApplet theApplet, float p0, float p1,
			String offPicture, String overPicture, int nbrCols) {
	 ...
	 */
	public GImageToggleButton(PApplet theApplet, float p0, float p1,
			String offPicture, String overPicture, int nbrCols, int nbrRows) {
		...
		if (nbrCols < 1 || nbrRows < 1 || offPicture == null
			...
			if (overPicture != null
					&& null != (temp = ImageManager.loadImage(winApp,
							overPicture))) {
				overImage = ImageManager.makeTiles1D(winApp, temp, nbrCols,
						nbrRows);
			}
		}
		...
		// ========================================================================
		// Setup the hotspots
		hotspots = new HotSpot[] { new HSalpha(1, 0, 0, offImage[stateValue],
				PApplet.CORNER) };
	 ...
	 * If the parameter is not a valid toggle state value then it is ignored and
	 * the button's state value is unchanged.
	 * 
	 * @param newState
	 */
	 ...
	 * For this toggle control I can't see the need for anything but CLICKED
	 * events
	 * 
	 * @param all
	 */
