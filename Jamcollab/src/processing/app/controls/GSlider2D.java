 ...
 * to set a different range for each axis independently. <b>
 * 
...
public class GSlider2D extends GValueControl2D {
	...
	public GSlider2D(PApplet theApplet, float p0, float p1, float p2, float p3) {
		...
		hotspots = new HotSpot[] {
				new HSrect(THUMB_SPOT, dragD - HALF_THUMB_SIZE + parametricPosX
						* dragWidth, dragD - HALF_THUMB_SIZE + parametricPosY
						* dragHeight, THUMB_SIZE, THUMB_SIZE), // thumb
		...
		createEventHandler(G4P.sketchApplet, "handleSlider2DEvents",
				new Class<?>[] { GSlider2D.class, GEvent.class }, new String[] {
						"slider2d", "event" });
	 ...
	 * The values supplied will be constrained to the appropriate current
	 * limits.
	 * 
	 * @param vx
	 *            the new X (horz) value
	 * @param vy
	 *            the new Y (vert) value
	 */
	...
	/**
	 * Get the current X value as a string taking into account the number
	 * format. <br>
	 */
	...
	/**
	 * Get the current Y value as a string taking into account the number
	 * format. <br>
	 */
