...
public class LoginPanel extends View {
	...
	public void Awake(boolean state) {
		StatusMessage.setVisible(isActive());
		LoginLabel.setVisible(isActive());
		LoginInput.setVisible(isActive());
		LoginButton.setVisible(isActive());
		LoginInput.setText("");
	...
	public void Start() {
		...
		LoginLabel = new GLabel(Application.app, 140, (needPass ? 160 : 200), 40, 20);
	...
	public void LoginButtonClick(GButton source, GEvent event) {
		...
		((HomePanel) ViewHandler.GetView("Home")).SwitchUser(user);
